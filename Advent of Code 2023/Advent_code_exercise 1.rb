...
game_and_sets_array.each_with_index do |game, index|
  game.map do |pair|
    ...
    if (hash["green"] && hash["green"] > 13) || (hash["red"] && hash["red"] > 12) || (hash["blue"] && hash["blue"] > 14)
      impossible_games << index + 1
    end
  end
end

result = possible_games.reject { |number| impossible_games.uniq.include?(number) }
