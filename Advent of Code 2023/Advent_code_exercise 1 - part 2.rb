...
game_and_sets_array = array.map { |game| game.split(":") }
                           .map { |game| game[1].split(/[;,]/) }

game_and_sets_array.each_with_index do |game, index|
  game.map do |pair|
    hash = { pair.match(/(\b\D+)/)[0].strip => pair.match(/\d+/)[0].to_i }
    if hash["green"]
      highest_greens << { (index + 1) => hash["green"] }
    elsif hash["red"]
      highest_reds << { (index + 1) => hash["red"] }
    elsif hash["blue"]
      highest_blues << { (index + 1) => hash["blue"] }
    end
  end
end
...
(1..100).to_a.each do |number|
  blue_values = group_by_highest_value(highest_blues)[number][number]
  green_values = group_by_highest_value(highest_greens)[number][number]
  red_values = group_by_highest_value(highest_reds)[number][number]
